...
int showEN = 0;			/* output in English */
...
void	Report(), ReportE(), ReportV(), ReportBM(), ReportGB();
void	usage();
void	Error(char*);
...
//char *argv[];
{
    ...
    if (!inverse && (year==SolarFirstDate.year) && (CmpDate(month, day, SolarFirstDate.month, SolarFirstDate.day))<0)
...
{
    printf("Usage:\n\n");
    printf("Solar->Lunar:\t%s [-e] [-h] [-b] year month day [hour]\n", progname);
    printf("\t\t(in Solar Calendar, 24 hour clock)\n\n");
    printf("Lunar->Solar:\t%s [-e] [-h] [-b] -i [-l] year month day [hour]\n",
	   progname);
    printf("\t\t(in Lunar Calendar, 24 hour clock)\n");
    printf("\t\t-l means the month is a leap month (\"run4 yue4\")\n\n");
	printf("\t\t-e means output in English\n");
...
void ReportV()
{
    ...
    printf("Âm Lịch : %d.%d%s.%d.%d\tCầm Tinh: %s\n",
	   lunar.year, lunar.month, (lunar.leap?"Nhuận":""), lunar.day,
	   lunar.hour, ConGiap[zhi.year] );
    ...
    printf("        (Trật Tự Can Chi)\t%d-%d.%d-%d.%d-%d.%d-%d\n",
	   gan.year+1, zhi.year+1, gan.month+1, zhi.month+1,
	   gan.day+1, zhi.day+1, gan.hour+1, zhi.hour+1);
    printf("        (Lục Thập Hoa Giáp)\t%d.%d.%d.%d\n\n",
	   GZcycle(gan.year, zhi.year), GZcycle(gan.month, zhi.month),
	   GZcycle(gan.day, zhi.day), GZcycle(gan.hour, zhi.hour));
    printf("Bát Tự theo 'Tính toán Tứ Trụ':\n");
    printf("        %s-%s.%s-%s.%s-%s.%s-%s\n",
...
//int i,j,k;
{
    ...
    for (int r=0; r<BMRow; r++)
    {
	for (int c=0; c<BMCol; c++) putchar(GZBM[i][r][c]);
	printf("  ");
	for (int c=0; c<BMCol; c++) putchar(GZBM[j][r][c]);
	printf("    ");
	for (int c=0; c<BMCol; c++) putchar(GZBM[k][r][c]);
	putchar('\n');
    }
    putchar('\n');
}
