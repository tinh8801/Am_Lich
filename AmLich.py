...
def jdFromDate(dd, mm, yy):
    ...
    y = yy + 4800 - a
    m = mm + 12*a - 3
    jd = dd + YNT((153*m + 2) / 5.) + 365*y + YNT(y/4) - YNT(y/100) + YNT(y/400) - 32045
    if (jd < 2299161):
        jd = dd + YNT((153*m + 2)/5) + 365*y + YNT(y/4) - 32083
    return jd
...
def jdToDate(jd):
    ...
    e = c - YNT((1461*d) / 4)
    m = YNT((5*e + 2) / 153)
    day = e - YNT((153*m + 2) / 5) + 1
    month = m + 3 - 12*YNT(m / 10)
    year = b*100 + d - 4800 + YNT(m / 10)
...
def getNewMoonDay(k, timeZone):
  ...
  ## Moon's argument of latitude
    ...
    JdNew = Jd1 + C1 - deltat
    return YNT(JdNew+0.5+timeZone/24)
...
def getSunLongitude(jdn, timeZone):
  ...
  ## mean longitude, degree
    ...
    L = L * dr
    L = L - math.pi*2*(YNT(L / (math.pi*2)))
  #### Normalize to (0, 2*math.pi)
    return YNT(L/math.pi*6)
...
def getLeapMonthOffset(a11, timeZone):
    ...
    k = YNT((a11 - 2415021.076998695) / 29.530588853 + 0.5)
    last = 0
...
def Lunar2Solar(lunarD, lunarM, lunarY, lunarLeap, tZ = 7):
    ...
    k = YNT(0.5 + (a11 - 2415021.076998695) / 29.530588853)
    off = lunarM - 11
    if (off < 0): off += 12
    if (b11 - a11 > 365):
        ...
        leapM = leapOff - 2
    if (leapM < 0): leapM += 12
    if (lunarLeap != 0 and lunarM != leapM): return [0, 0, 0]
    elif (lunarLeap != 0 or off >= leapOff): off += 1
...
def CanChi(dd, mm, yy):
    ...
    if(dd < 0 | dd > 31 | mm < 0 | mm > 12 | yy < 0): print('Ngày, tháng, năm không hợp lệ')
    else:
        lDate=Solar2Lunar(dd,mm,yy)
        lYear=lDate[2]
        canIndex=lYear % 10
        chiIndex=lYear % 12
...
if __name__=='__main__':
    ...
    print("Ngày Dương Lịch: ", str(currentDay)+"-"+str(currentMonth)+"-"+str(currentYear))
    kq=Solar2Lunar(currentDay, currentMonth, currentYear)
    #kq=S2L(22, 3, 2023) #day la ngay 1/2 am lich nhuan cua nam 2023
    thangnhuan=None
    if kq[3]==0:
        thangnhuan=""
    else:
        thangnhuan="(nhuận)"
    print("Ngày Âm Lịch: ", str(kq[0])+"-"+str(kq[1])+ thangnhuan+"-"+str(kq[2])+" ("+CanChi(currentDay,currentMonth,currentYear)+")")
    kq=Solar2Lunar(25, 7, 2025)
    if kq[3]==0:
        thangnhuan=""
    else:
        thangnhuan=" (nhuận)"
    print("Ngày Âm Lịch:", str(kq[0])+"-"+str(kq[1])+thangnhuan+"-"+CanChi(currentDay,currentMonth,currentYear))
    print("là ngày: ", Lunar2Solar(1, 6, 2025, 1, 7))