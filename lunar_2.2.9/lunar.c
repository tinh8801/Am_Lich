...
static	char	*GanB5[] = {
    "\xA5\xD2", "\xA4\x41", "\xA4\xFE", "\xA4\x42", "\xA5\xB3",
    "\xA4\x76", "\xA9\xB0", "\xA8\xAF", "\xA4\xD0", "\xAC\xD1"
};

static	char	*ZhiB5[] = {
    "\xA4\x6C", "\xA4\xA1", "\xB1\x47", "\xA5\x66", "\xA8\xB0", "\xA4\x78",
    "\xA4\xC8", "\xA5\xBC", "\xA5\xD3", "\xA8\xBB", "\xA6\xA6", "\xA5\xE8"
};

static	char   *ShengXiaoB5[] = {
    "\xB9\xAB", "\xA4\xFB", "\xAA\xEA", "\xA8\xDF", "\xC0\x73", "\xB3\x44",
    "\xB0\xA8", "\xA6\xCF", "\xB5\x55", "\xC2\xFB", "\xAA\xAF", "\xBD\xDE"
};

static char *weekdayB5[] = {
    "\xA4\xE9", "\xA4\x40", "\xA4\x47", "\xA4\x54",
    "\xA5\x7C", "\xA4\xAD", "\xA4\xBB"
};
...
char	BMfile[] = "lunar.bitmap";//"/usr/share/lunar/lunar.bitmap";	/* bit map file */
char	GZBM[NBM][BMRow][BMCol];	/* the bitmap array */
char	*progname;

void	Solar2Lunar(), Lunar2Solar();
long    Solar2Day(Date*), Solar2Day1(Date*), Lunar2Day(Date*);
void	Day2Lunar(long, Date*), Day2Solar(long, Date*);
int     make_yday(), make_mday(int), GZcycle(int, int);
void	CalGZ(long, Date*, Date*, Date*);
int		CmpDate(int, int, int, int), JieDate(Date*, Date*);
void    readBM(), display3(int, int, int);
void	Report(), ReportE(), ReportV(), ReportBM(), ReportGB(), ReportB5();
void	ReportUTF8_hant(), ReportUTF8_hans();
void	usage(), Error(char*);
...
{
    printf("Usage:\n\n");
    printf("Solar->Lunar:\t%s [-e] [-u] [-h] [-b] [year month day [hour]]\n", progname);
    printf("\t\t(in Solar Calendar, 24 hour clock)\n\n");
    printf("Lunar->Solar:\t%s [-e] [-u] [-h] [-b] -i [-l] year month day [hour]\n",
	   progname);
    printf("\t\t(in Lunar Calendar, 24 hour clock)\n");
    printf("\t\t-l  means the month is a leap month (\"run4 yue4\")\n\n");
	printf("\t\t-e          means output English\n");
...
void Day2Lunar(long offset, Date *d)
//long offset;
//Date *d;
{
...
void Day2Solar(long offset, Date *d)
//long offset;
//Date *d;
{
    int	i, m, days;
...
void ReportB5()
{
    printf("%s%d%s%2d%s%2d%s%2d%s%s%s\n", "\xB6\xA7\xBE\xE4\xA1\x47\xA1\x40",
	   solar.year, "\xA6\x7E", solar.month, "\xA4\xEB", solar.day,
	   "\xA4\xE9", solar.hour, "\xAE\xC9\xA1\x40",
	   "\xAC\x50\xB4\xC1", weekdayB5[solar.weekday]);
    printf("%s%d%s%s%2d%s%2d%s%s%s%s%s\n", "\xB3\xB1\xBE\xE4\xA1\x47\xA1\x40",
	   lunar.year, "\xA6\x7E", (lunar.leap? "\xB6\x7C":""),
	   lunar.month, "\xA4\xEB", lunar.day, "\xA4\xE9",
	   ZhiB5[zhi.hour], "\xAE\xC9\xA1\x40",
	   "\xA5\xCD\xA8\x76\xC4\xDD", ShengXiaoB5[zhi.year]);
    printf("%s%s%s%s%s%s%s%s%s%s%s%s%s\n", "\xA4\x7A\xA4\xE4\xA1\x47\xA1\x40",
	   GanB5[gan.year], ZhiB5[zhi.year], "\xA6\x7E\xA1\x40",
	   GanB5[gan.month], ZhiB5[zhi.month], "\xA4\xEB\xA1\x40",
	   GanB5[gan.day], ZhiB5[zhi.day], "\xA4\xE9\xA1\x40",
	   GanB5[gan.hour], ZhiB5[zhi.hour], "\xAE\xC9\xA1\x40");
    printf("%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
	   "\xA5\xCE\xA5\x7C\xAC\x57\xAF\xAB\xBA\xE2\xB1\xC0\xBA\xE2\xA4\xA7\xAE\xC9\xA8\xB0\xA4\x4B\xA6\x72\xA1\x47\xA1\x40",
	   GanB5[gan2.year], ZhiB5[zhi2.year], "\xA6\x7E\xA1\x40",
	   GanB5[gan2.month], ZhiB5[zhi2.month], "\xA4\xEB\xA1\x40",
	   GanB5[gan2.day], ZhiB5[zhi2.day], "\xA4\xEB\xA1\x40",
	   GanB5[gan2.hour], ZhiB5[zhi2.hour], "\xAE\xC9\xA1\x40");
    if (jieAlert)
    {
	printf("* %s, %s\n", "\xAC\x4F\xA4\xE9\xAC\xB0\xB8\x60",
	       "\xA4\xEB\xAC\x57\xA5\x69\xAF\xE0\xAD\x6E\xAD\xD7\xA7\xEF");
	if (lunar2.month==1)
	    printf("* %s\n", "\xA6\x7E\xAC\x57\xA5\xE7\xA5\x69\xAF\xE0\xAD\x6E\xAD\xD7\xA7\xEF");
	printf("* %s\n", "\xBD\xD0\xAC\x64\xA6\xB3\xB8\x60\xAE\xF0\xAE\xC9\xB6\xA1\xA4\xA7\xB8\x55\xA6\x7E\xBE\xE4");
    }
}
...
void ReportV()
{
    printf("Dương lịch : %d.%d.%d.%d\t%s\n", solar.year, solar.month, solar.day,
	   solar.hour, weekdayV[solar.weekday]);
    printf("Âm lịch : %d.%d%s.%d.%s\tCầm tinh: %s\n",
	   lunar.year, lunar.month, (lunar.leap?" Nhuận":""), lunar.day,
	   Chi[zhi.hour], ConGiap[zhi.year] );
    printf("Can Chi: %s-%s.%s-%s.%s-%s.%s-%s\n",
	   Can[gan.year], Chi[zhi.year], Can[gan.month], Chi[zhi.month],
	   Can[gan.day], Chi[zhi.day], Can[gan.hour], Chi[zhi.hour]);
    printf("        (Thứ tự Can Chi)\t%d-%d.%d-%d.%d-%d.%d-%d\n",
	   gan.year+1, zhi.year+1, gan.month+1, zhi.month+1,
	   gan.day+1, zhi.day+1, gan.hour+1, zhi.hour+1);
    printf("        (Lục Thập Hoa Giáp)\t%d.%d.%d.%d\n\n",
	   GZcycle(gan.year, zhi.year), GZcycle(gan.month, zhi.month),
	   GZcycle(gan.day, zhi.day), GZcycle(gan.hour, zhi.hour));
    printf("Bát tự dựa theo 'Tính toán Tứ Trụ':\n");
    printf("        %s-%s.%s-%s.%s-%s.%s-%s\n",
	   Can[gan2.year], Chi[zhi2.year], Can[gan2.month], Chi[zhi2.month],
	   Can[gan2.day], Chi[zhi2.day], Can[gan2.hour], Chi[zhi2.hour]);
    printf("        (Thứ tự Can Chi)\t%d-%d.%d-%d.%d-%d.%d-%d\n",
	   gan2.year+1, zhi2.year+1, gan2.month+1, zhi2.month+1,
	   gan2.day+1, zhi2.day+1, gan2.hour+1, zhi2.hour+1);
    printf("        (Lục Thập Hoa Giáp)\t%d.%d.%d.%d\n\n",
...
void display3(int i, int j, int k)
//int i,j,k;
{
    int	r,c;
